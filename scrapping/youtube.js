...
const shortUrl = async (url) => {
	return (await axios.get(`https://tinyurl.com/api-create.php?url=${encodeURIComponent(url)}`)).data;
};
...
class Youtube {
					...
					for (const getData of searchData) {
						...
						if (videosRenderer) {
							const resultData = {
								...
								thumbnail: videosRenderer.thumbnail.thumbnails[1]?.url.split("?")[0] ?? videosRenderer.thumbnail.thumbnails[0]?.url.split("?")[0] ?? "https://api-xcoders.xyz/images/avatar.png",
								duration: videosRenderer.thumbnailOverlays[0].thumbnailOverlayTimeStatusRenderer?.text.simpleText.replace(".", ":") || "Unknown",
								published_at: videosRenderer.publishedTimeText?.simpleText || "Unknown",
		...
		return new Promise(async (resolve, reject) => {
			await axios.request({
				...
				if (arguments[1] == false) {
					const filterMetadataVideo = (data.url).filter(fill => fill.quality.toString() === "360" ?? "720");
					const chooseMetadataVideo = filterMetadataVideo[0] ?? filterMetadataVideo[1] ?? filterMetadataVideo[2] ?? filterMetadataVideo[3];
					...
					const urlMetadataVideo = chooseMetadataVideo.url ?? chooseMetadataVideo.info_url;
					...
					const filterMetadataAudio = (data.url).filter(fill => fill.audio === true);
					const chooseMetadataAudio = filterMetadataAudio[0] ?? filterMetadataAudio[1] ?? filterMetadataAudio[2] ?? filterMetadataAudio[3];
					...
					const urlMetadataAudio = chooseMetadataAudio.url ?? chooseMetadataAudio.info_url;
	...
	play(query, mp3 = false) {
		return new Promise(async (resolve, reject) => {
			...
			if ((argumentUrl && argumentUrl[0] && argumentUrl[0].url) === undefined) return reject("error searching data");
			await axios.request({
				method: "POST",
				url: "https://api.onlinevideoconverter.pro/api/convert",
	...
	short(url) {
		return new Promise(async (resolve, reject) => {
			...
			const link = 'https://www.youtube.com/watch?v='+ pathUrl.pathname.split('/')[2];
