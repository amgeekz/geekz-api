...
class Information {
	...
	Searchnabi(query) {
		return new Promise(async (resolve, reject) => {
			try {
				let data = await fs.readFileSync(process.cwd() + `/data/islamic/kisahnabi/${query.toLowerCase()}.json`);
			...
			await axios.get(`http://api.openweathermap.org/data/2.5/weather?q=${kota}&units=metric&appid=132d0c03530775bbbfa9b298392866df`)
				.then(async ({ data }) => {
					...
					if (data.cod == 404) {
						...
						let sunrise = await moment.tz(data.sys.sunrise * 1000, "Asia/Jakarta").format("DD, MM - yy : HH:mm") + " WIB";
						let sunset = await moment.tz(data.sys.sunset * 1000, "Asia/Jakarta").format("DD, MM - yy : HH:mm") + " WIB";
						const result = {
	...
	trendtweet(query) {
		return new Promise(async (resolve, reject) => {
			await axios.get(`https://twitter-trends.iamrohit.in/${query.toLowerCase()}`)
				.then(({ data }) => {
					if (isAxiosError()) throw ('axios error');
					let hasil = new Array();
					const $ = cheerio.load(data);
					...
					$("tbody#copyData > tr").each(function (a, b) {
						hasil.push({
							trending_number: $(this).find("th:nth-child(2) > a").attr("rank") || undefined,
							hastag: $(this).find("th:nth-child(2) > a").text().trim(),
							tweet_count: $(this).find("th:nth-child(2) > div > span").text().trim(),
	...
	trendgoogle(query) {
		return new Promise(async (resolve, reject) => {
			...
			await axios.get(`https://gtrends.iamrohit.in/${str + str2}`).then(({ data }) => {
				if (isAxiosError()) throw ('axios error');
				let result = new Array();
				const $ = cheerio.load(data);
				const message = $("html > head > title").text().trim();
				const updated_at = $("div.xs-12:nth-child(2) > div > div > div > h1 > span").text().trim();
				$("div.xs-12:nth-child(2) > div > div.row").each(function (a, b) {
					result.push({
						title: $(this).find("div.col-md-9 > ul > li > a").text().trim() || undefined,
		...
		return new Promise(async (resolve, reject) => {
			await axios.get(url).then(({ data }) => {
				...
				$("div.md-9 > div > div.row").each(function (a, b) {
					result.push({
						title: $(this).find("div.col-md-4 > a > img").attr("title") || undefined,
						thumbnail: $(this).find("div.col-md-4 > a > img").attr("src"),
						url: $(this).find("div.col-md-4 > a").attr("href"),
						channel: $(this).find("div.col-md-8 > p:nth-child(2) > b:nth-child(1)").text().trim(),
						uploaded_at: $(this).find("div.col-md-8 > p:nth-child(2) > b:nth-child(2)").text().trim(),
						viewers: $(this).find("div.col-md-8 > p:nth-child(3) > b:nth-child(2)").text().trim() + " Views",
						likes: $(this).find("div.col-md-8 > p:nth-child(3) > b:nth-child(4)").text().trim() + " Likes",
						dislikes: $(this).find("div.col-md-8 > p:nth-child(3) > b:nth-child(6)").text().trim() + " Dislike",
						comments: $(this).find("div.col-md-8 > p:nth-child(3) > b:nth-child(8)").text().trim() + " Comment",
						description: $(this).find("div.col-md-8 > p:nth-child(5)").text().trim(),
					});
				});
				resolve({
					message,
