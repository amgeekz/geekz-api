...
class Tools {
	...
	bibbleDays() {
		return new Promise(async (resolve, reject) => {
			await axios.get('https://www.bible.com/id/verse-of-the-day').then(({ data }) => {
				if (isAxiosError()) throw 'Axios Error';
				const $ = cheerio.load(data);
				const title = $("amp-img.br2.img-wrap").attr("alt");
				const date = $("p.f5.fw4.yv-gray25.mt0.mb0").text();
				const thumbnail = "https://" + $("amp-img.br2.img-wrap").attr("src").split("https://")[1];
				const url = "https://www.bible.com" + $("div.relative.img-wrap > a").attr("href");
	...
	async distance(from, to) {
		...
		const execFrom = /onclick='addValue1\(this.getAttribute\("rel"\), "(.*?)"\)'/.exec(getFrom.data)[1];
		const execTo = /onclick='addValue1\(this.getAttribute\("rel"\), "(.*?)"\)'/.exec(getTo.data)[1];
		...
		try {
			const getDistance = await axios.request({
				...
				headers: {
					'User-Agent': 'Mozilla/5.0 (Linux; Android 9; CPH1923) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.101 Mobile Safari/537.36',
			...
			result.distance = /koordinat — (.*?)mil./g.exec(getInformation)[1] + 'mil';
	...
	sfileSearch(query) {
		return new Promise(async (resolve, reject) => {
			await axios.get(`https://sfile.mobi/search.php?q=${query}&search=Search`).then(({ data }) => {
				const $ = cheerio.load(data);
	...
	apkmirror(query) {
		return new Promise(async (resolve, reject) => {
			await axios.get('https://www.apkmirror.com/?post_type=app_release&searchtype=apk&s=' + query).then(({ data }) => {
				if (isAxiosError()) throw 'Axios error';
				const $ = cheerio.load(data);
				let title = new Array();
	...
	async randomGoore() {
		const getData = await axios.get('https://deepgoretube.site/page/' + Math.floor(Math.random() * 5));
		if (isAxiosError()) throw 'Axios error';
		const $ = cheerio.load(getData.data);
		let data = new Array();
		$("body > section.btp_inner_page > div > div > div > div > div").each(function (a, b) {
			const thumbnail = $(this).find("img.btp_post_card__img").attr("src");
			const title = $(this).find("img.btp_post_card__img").attr("alt");
			const quality = $(this).find("div.card-img-overlay > div:nth-child(1)").text().split(/\n| /g).join("");
			const duration = $(this).find("div.card-img-overlay > div:nth-child(2)").text().split(/\n| /g).join("");
			const url = $(this).find("a.btp_post_card__title").attr("href");
			const authorname = $(this).find("a.btp_post_card__author > span.btp_post_card__author_text").text().split(/\n| /g).join("");
			const author_url = $(this).find("a.btp_post_card__author").attr("href");
			const published = $(this).find("small.mr-2").text().split(/\n/g).join("").trim();
			data.push({
				title,
				authorname,
				author_url,
	...
	async getGoore(link) {
		if (!/https?:\/\/deepgoretube\.site\/[A-Za-z0-9-_]+/.test(link)) throw 'invalid url';
		const getData = await axios.get(link);
		if (isAxiosError()) throw 'Axios error';
		const $ = cheerio.load(getData.data);
		const getVideo = /'encrypt:(.*?)'/g.exec(getData.data)[1];
		const metadata = $("script.yoast-schema-graph").get().map(a => {
			let response;
		...
		let likes = $("span.wp_ulike_counter_up").attr("data-ulike-counter-value").split("+").join("") + " like";
		let dislikes = $("span.wp_ulike_counter_down").attr("data-ulike-counter-value") + " dislike";
	...
	text2speach(text, language) {
		return new Promise(async (resolve, reject) => {
			...
			await axios.request({
				...
				data: { "engine": "Google", "data": { "text": text, "voice": language + "-" + twoLG } },
				headers: {
				...
				if (isAxiosError()) throw ('axios error');
				const getData = await axios.get(`https://api.soundoftext.com/sounds/${response.data.id}`);
				if (isAxiosError()) throw ('axios error');
	...
	async pinterest(query) {
		let HASIL = new Array();
		await axios.request(`https://id.pinterest.com/search/pins/?rs=typed&q=` + query, {
			method: "GET",
	...
	imageSearch(query) {
		return new Promise(async (resolve, reject) => {
			await axios.get(`https://images.search.yahoo.com/search/images;_ylt=Awr9F68TYZlhbvoAGsWLuLkF;_ylc=X1MDOTYwNTc0ODMEX3IDMgRmcgMEZ3ByaWQDNjJIOTFIZjBUYkNuek4xendqT09BQQRuX3N1Z2cDNQRvcmlnaW4DaW1hZ2VzLnNlYXJjaC55YWhvby5jb20EcG9zAzAEcHFzdHIDBHBxc3RybAMEcXN0cmwDNwRxdWVyeQNzb2xlaG90BHRfc3RtcAMxNjM3NDQyMDc1?fr2=sb-top-images.search&p=${encodeURIComponent(query)}&ei=UTF-8&fr=sfp&bucket=&exclusive_bucket=`)
				.then(async ({ data }) => {
					if (isAxiosError()) throw ('axios error');
					const $ = cheerio.load(data);
					let result = new Array();
					$("div.sres-cntr").each(function (a, b) {
						$(this).find("ul#sres > li").each(function (c, d) {
							result.push({
	...
	dorking(query) {
		return new Promise(async (resolve, reject) => {
			await axios.request({
				method: "GET",
				url: `https://www.google.com/search?q=${encodeURIComponent(query)}&oq=${encodeURIComponent(query)}&aqs=chrome..69i57j69i58.749j0j4&ie=UTF-8`,
				headers: {
					"User-Agent": fakeUa()
				}
			})
	...
	cekCC(data) {
		return new Promise(async (resolve, reject) => {
			await axios.request({
				...
				resolve($("div").text().split("@")[0] || "Check the validity of a credit card");
			}).catch(reject);
		});
	}
