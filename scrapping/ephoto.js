...
class Ephoto {
	...
	text(res, url, text) {
		return new Promise(async (resolve, reject) => {
			await axios.request({ method: "GET", url }).then(async resultOne => {
				...
				$("form.ajax-submit").find("input").get().map(mappingData => {
					inputData.push({
						name: $(mappingData).attr("name"),
						value: $(mappingData).attr("value")
					});
				});
				const filterInputData = inputData.filter(filtering => filtering.name !== "text[]");
				if (typeof text == "string") text = [text];
				for (let teks of text) formDataAppend.append("text[]", teks);
				formDataAppend.append(filterInputData[0].name, filterInputData[0].value);
				formDataAppend.append(filterInputData[1].name, filterInputData[1].value);
				formDataAppend.append(filterInputData[2].name, filterInputData[2].value);
				formDataAppend.append(filterInputData[3].name, filterInputData[3].value);
	...
	image(res, url, link, text = ["Fxc7", "RestAPIs"], agent) {
		return new Promise(async (resolve, reject) => {
			...
			await jimp.read(link).then(image => {
				...
				image.getBuffer(image._originalMime, async function (error, response) {
					if (error) return reject(error);
					buffer = response;
				});
			}).catch(() => functions.resError(res, "failed loaded image, please input url image correctly"));
			...
			await axios.request({
				...
				await axios.request({
					...
					if (isAxiosError()) throw ('axios error');
					let getToken = /name="form_value_input".*value="(.+?)"/.exec(resultTwo.data)[1];
	...
	radio_text(res, url, text) {
		return new Promise(async (resolve, reject) => {
			...
			let inputData = new Array();
			$("form.ajax-submit").find("input").get().map(mapData => {
				inputData.push({
					name: $(mapData).attr("name"),
					value: $(mapData).attr("value")
				});
			});
			const inputDataFilterRadio = inputData.filter(fill => fill.name == "radio0[radio]");
			const inputDataFilter = inputData.filter(fill => fill.name !== "radio0[radio]");
			const inputDataFilterText = inputDataFilter.filter(fill => fill.name !== "text[]");
			const randomFilterInput = inputDataFilterRadio[Math.floor(Math.random() * inputDataFilterRadio.length)];
			let formData = new FormData();
			for (let i = 0; i < inputDataFilterText.length; i++) {
				formData.append(inputDataFilterText[i].name, inputDataFilterText[i].value);
			...
			await axios.request({
				...
				if (isAxiosError()) throw ('axios error');
				const getToken = /name="form_value_input".*value="(.+?)"/.exec(resultOne.data)[1];
				const getServerId = /name="build_server".*value="(.+?)"/.exec(getCookie.data)[1];
