	...
	Search(query) {
		return new Promise(async (resolve, reject) => {
			...
			await axios({
				...
				headers: { "User-Agent": "Mozilla/5.0 (Linux; Android 9; CPH1923) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/93.0.4577.62 Mobile Safari/537.36", "Cookie": "csrftoken=17TSy54cYatlvfQsLvjc7yJxaGlBskEJYKgMuOjO1Q0BKXamQ8BHDvZRUt8RCQUH" }
			}).then(({ data }) => {
	...
	Code(code) {
		return new Promise(async (resolve, reject) => {
			...
			await axios({
				...
				if (isAxiosError()) throw ('axios error');
				const $ = cheerio.load(data);
				const titleID = $("div#info > h1.title > span.before").text() + " " + $("div#info > h1.title > span.pretty").text() + " " + $("div#info > h1.title > span.after").text();
				const titleJP = $("div#info > h2.title > span.before").text() + " " + $("div#info > h2.title > span.pretty").text() + " " + $("div#info > h2.title > span.after").text();
				const thumbnail = $("div#cover > a > img").attr("data-src");
				const code = $("div#info > h3#gallery_id").text().trim();
				const parodies = $("div#info > section#tags > div").text().trim().split(/\n/g).map(obj => obj.replace(/\t/g, ''));
				const meta = [
		...
		return new Promise(async (resolve, reject) => {
			...
			await axios({
				...
				await axios({
					...
					const thumbnail = $("div#cover > a > img").attr("data-src");
					const code = $("div#info > h3#gallery_id").text().trim();
					const parodies = $("div#info > section#tags > div").text().replace(/\t/gi, "").split(/\n/gi);
					const meta = [
