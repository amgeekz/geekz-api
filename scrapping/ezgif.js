...
class Ezgif {
	...
	toMp4(url) {
		return new Promise(async (resolve, reject) => {
			await axios.get(`https://ezgif.com/webp-to-mp4?url=${url}`)
				.then(async ({ data }) => {
	...
	toWebp(url) {
		return new Promise(async (resolve, reject) => {
			await axios.get(`https://ezgif.com/png-to-webp?url=${url}`)
				.then(async ({ data }) => {
					if (isAxiosError()) throw ('axios error');
					const $ = cheerio.load(data);
					...
					await axios.request({
						...
						if (isAxiosError()) throw ('axios error');
						const $ = cheerio.load(data);
						const result = "https:" + $('div[id="output"]').find("p").first().find("img").attr("src");
						resolve(result);
					}).catch(reject);
				}).catch(reject);
		});
	...
	reverseVideo(url) {
		return new Promise(async (resolve, reject) => {
			await axios.get(`https://ezgif.com/reverse-video?url=${url}`)
				.then(async ({ data }) => {
					if (isAxiosError()) throw ('axios error');
					const $ = cheerio.load(data);
					const dataValue = $('form > input').get().map(a => {
						return {
							name: $(a).attr('name'),
							value: $(a).attr('value')
						};
