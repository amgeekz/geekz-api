	...
	text(url, text) {
		return new Promise(async (resolve, reject) => {
			...
			await axios.request({
				...
				$("form.ajax-submit").find("input").get().map(mappingInput => {
					dataFromResult.push({
				...
				FormDataAppend.append(dataResultFilter[0].name, dataResultFilter[0].value);
				FormDataAppend.append(dataResultFilter[1].name, dataResultFilter[1].value);
				FormDataAppend.append(dataResultFilter[2].name, dataResultFilter[2].value);
				FormDataAppend.append(dataResultFilter[3].name, dataResultFilter[3].value);
				await axios.request({
					method: "POST",
					url,
					data: FormDataAppend,
