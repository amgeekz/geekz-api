...
class Games {
	...
	tebakGambar() {
		return new Promise(async (resolve, reject) => {
			...
			await axios.get(`https://jawabantebakgambar.net/level-${random_level}/`, { headers: { "User-Agent": fakeUa() } })
				.then(({ data }) => {
	...
	asahotak() {
		return new Promise((resolve, reject) => {
			axios.get('https://www.cademedia.com/jawaban-tebak-tebakan-asah-otak', { headers: { "User-Agent": fakeUa() } })
				.then(res => {
					if (isAxiosError()) throw ('axios error');
					...
					for (let i = 2; i < go.length; i++) {
						...
						switch (i) {
							case 229:
								y = (go[i].split('J:')[1].split('Level')[0].split('Demikian')[0]).trim();
								break;
							default:
	...
	family100() {
		return new Promise((resolve, reject) => {
			axios.get('https://teknologital.com/kunci-jawaban-ica-ica', { headers: { "User-Agent": fakeUa() } })
				.then(res => {
					if (isAxiosError()) throw ('axios error');
					const $ = cheerio.load(res.data);
					let data = new Array();
					let y;
					const go = $('body').find('article.blog-post > div.row > div').text().split('Jawaban :');
	...
	siapakah() {
		return new Promise((resolve, reject) => {
			axios.get('https://tutorialaplikasi.com/kunci-jawaban-tebak-siapakah-aku/', { headers: { "User-Agent": fakeUa() } })
				.then(res => {
					if (isAxiosError()) throw ('axios error');
					const $ = cheerio.load(res.data);
					let data = new Array();
	...
	siapakah2() {
		return new Promise((resolve, reject) => {
			axios.get('https://jagat-nusantara.blogspot.com/2018/01/kunci-jawaban-tebak-siapakah-aku-m.html', { headers: { "User-Agent": fakeUa() } })
				.then(res => {
					if (isAxiosError()) throw ('axios error');
					const $ = cheerio.load(res.data);
					let data = new Array();
					...
					for (let i = 1; i < 535; i++) {
						...
						x = go.split(i + '.')[1].split("Jawaban: ")[1].split("\n")[0];
						data.push({
							pertanyaan: y,
							jawaban: x
						});
	...
	susunkata() {
		return new Promise((resolve, reject) => {
			axios.get('https://www.cademedia.com/jawaban-tebak-tebakan-susun-kata', { headers: { "User-Agent": fakeUa() } })
				.then(res => {
					if (isAxiosError()) throw ('axios error');
					const $ = cheerio.load(res.data);
					let data = new Array();
	...
	tekateki() {
		return new Promise((resolve, reject) => {
			axios.get('https://www.kabargames.id/kunci-jawaban-tebak-tebakan-2020', { headers: { "User-Agent": fakeUa() } })
				.then(({ data }) => {
					if (isAxiosError()) throw ('axios error');
					const $ = cheerio.load(data);
	...
	cakLontong() {
		return new Promise((resolve, reject) => {
			axios.get("https://gamedaim.com/tips/kunci-jawaban-tts-cak-lontong-2020/amp/", { headers: { "User-Agent": fakeUa() } })
				.then(({ data }) => {
					if (isAxiosError()) throw ('axios error');
					const $ = cheerio.load(data);
					let result = new Array();
					$("div.amp-wp-article-content").each(function (a, b) {
						const xx = $(this).find("p").next().next().next().next().next().next().next().text();
						for (let i = 1; i < 100; i++) {
							result.push({
								pertanyaan: xx.split(/Pertanyaan:/gi)[i].split(/Jawaban:/gi)[0],
								jawaban: xx.split(/Jawaban:/gi)[i].split(/Keterangan:/gi)[0],
								keterangan: xx.split(/Keterangan:/gi)[i].split(/Pertanyaan:/gi)[0]
							});
						}
					});
					resolve(result);
