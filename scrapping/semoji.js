...
class Sticker {
	...
	async search(query) {
		...
		$("#stickerPacks > div > div.row.justify-content-center > div.sticker-pack-cols").each(function (a, b) {
			result.push({
	...
	async toSticker(query) {
		const resmojipedia = await axios.get(`https://emojipedia.org/search/?q=${encodeURIComponent(query)}`);
		if (isAxiosError()) throw ('axios error');
