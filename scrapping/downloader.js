	...
	cocofun(url) {
		return new Promise((resolve, reject) => {
			axios.request({
				...
				const result = {
					title: getData.content + " #" + getData.topic.topic || getData.topic.topic,
		...
		return new Promise((resolve, reject) => {
			axios.request(url, {
				...
				const resulter = {
					...
					url: $("div.__gestaltThemeVideo > video.hwa").attr("src").replace(/hls/gi, "720p").replace(/m3u8/gi, "mp4")
				};
				resolve(resulter);
	...
	stickerline(url) {
		return new Promise((resolve, reject) => {
			axios.get(url).then(async response => {
				...
				const $ = cheerio.load(response.data);
				let hasil = new Array();
				$("div.mdCMN09ImgListWarp ul.mdCMN09Ul > li.mdCMN09Li").each(function (a, b) {
