...
class ameClient {
	...
	constructor(token, options = {}) {
		if (!token) throw ("Unknown Token: Token Missing");
	...
	async generate(endpoint, data = {}) {
		...
		try {
			let image = await axios.post(`${this.baseURL}/generate/${endpoint}`, data, {
	...
	async getEndpointsGenerate(onlyFree = false) {
		try {
			let info = await axios.get(`${this.baseURL}/generate`);
	...
	async getEndpointsImage(onlyFree = false) {
		try {
			let info = await axios.get(`${this.baseURL}/image`);
			if (!info.data) throw "Endpoints not found.";
			if (isAxiosError()) throw ('axios error');
			return onlyFree ? info.data.endpoints.free : [...info.data.endpoints.free, ...info.data.endpoints.premium];
		} catch (e) {
