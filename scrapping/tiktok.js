...
class Tiktok {
	...
	async getHashtagVideo(tag) {
		...
		let data;
		if (sortBy == 'date') {
			data = results.sort((a, b) => new Date(a.createTime) - new Date(b.createTime));
		} else if (sortBy == 'views') {
			data = results.sort((a, b) => a.stats.playCount - b.stats.playCount);
		} else if (sortBy == 'followers') {
			data = results.sort((a, b) => a.authorStats.followerCount - b.authorStats.playCount);
		...
		for (let i = 0; i < data.length; i++) {
			result.push({
				...
				uploaded: moment(data[i].createTime * 1000).tz("Asia/Jakarta").locale("id").format(" HH - MM - yy"),
		...
		return new Promise(async (resolve, reject) => {
			try {
				...
				await axios.request({
					...
					data: `url=${encodeURIComponent(link.origin + link.pathname)}&token=${tokenID}`,
					headers: {
	...
	savetik(url) {
		return new Promise((resolve, reject) => {
			...
			axios.request({
				method: "POST",
				url: baseURL,
				data: new URLSearchParams(Object.entries({ "tiktok_url": link.origin + link.pathname }))
			}).then(({ data }) => {
				if (isAxiosError()) throw ('axios error');
				const $ = cheerio.load(data);
				const result = {
