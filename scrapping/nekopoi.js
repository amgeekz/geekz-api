...
class Nekopoi {
	constructor() {
		...
		this.cookie = "__cf_bm=c3a3z0cTHCsxap_ks1FD9XX7D2zBTybrQl7GaWQ.eJo-1640194143-0-ARy1I2sbY7e8ToZtnf8rUhRYIYYpIzI0Z0oiLuwmVhcPM2jS0sPP1kigMP9tvyQnyfX4Hjact05eX/5WNuDHCFTKxX0ThS7uBSmyb9YxFY8YeThNnm/HZZJrM8nUT/F+YQ==";
	}
	Latest() {
	...
	Search(query) {
		return new Promise(async (resolve, reject) => {
			await axios.request({
				...
				const $ = cheerio.load(data);
				let result = [];
				$("div.postsbody > div.result").find("div.top").each(function (a, b) {
					result.push({
	...
	Get(url, response) {
		return new Promise(async (resolve, reject) => {
			if (!isLink(url)) return resError(response, "invalid url, make sure the url is correct");
			await axios.request({
				method: "GET",
				url: encodeURI(url)
			}).then(({ data }) => {
				...
				let meta = $("div.contentpost > div.konten").text().split(" : ").join("\n").split("\n");
				const metadata = [
					{ movieID: meta[3] },
				...
				let linkDownload = [];
				$("div.boxdownload > div.liner:nth-child(1) > div.listlink").find("a").each(function (a, b) {
					linkDownload.push({ [$(this).text().toLowerCase().split(" ")[0]]: $(this).attr("href") });
				});
				resolve({
					title,
					thumbnail,
