...
router.get("/quotes", apikeyAndLimit, async (req, res) => {
	...
	await getJson(`https://canvacord-production.up.railway.app/api/quotes?text=${encodeURIComponent(text)}&wm=${encodeURIComponent(wm)}&url=${background}`).then(async response => {
...
router.get("/rmbg", apikeyAndLimit, async (req, res) => {
	...
	await getBuffer(url).then(async buffer => {
		if (!Buffer.isBuffer(buffer)) return resError(res, "not a buffer type");
		const fileType = await fromBuffer(buffer);
		if (fileType.ext !== "jpg") return resError(res, "file type can't " + fileType.ext);
		await tools.removebg(buffer).then(async buffer => {
			await getBuffer(buffer).then(response => res.type("png").send(response))
				.catch(e => {
					console.error(e);
					res.json(loghandler.error);
				});
...
router.get("/badut", apikeyAndLimit, async (req, res) => {
	...
	if (!url) return res.json(loghandler.noturl);
	if (!regexUrl(url)) return res.json(loghandler.urlInvalid);
	buffer = await getBuffer(`https://canvacord-production.up.railway.app/api/badut?url=${encodeURIComponent(url)}`);
	res.type("png");
	res.send(buffer);
});
router.get("/badut2", apikeyAndLimit, async (req, res) => {
	...
	if (!url) return res.json(loghandler.noturl);
	if (!regexUrl(url)) return res.json(loghandler.urlInvalid);
	buffer = await getBuffer(`https://canvacord-production.up.railway.app/api/badut2?url=${encodeURIComponent(url)}`);
	res.type("png");
	res.send(buffer);
});
...
router.get("/twitter", apikeyAndLimit, async (req, res) => {
	...
	if (!text) return res.json(loghandler.nottext);
	if (!regexUrl(url)) return res.json(loghandler.urlInvalid);
	await getBuffer(`https://canvacord-production.up.railway.app/api/twitter?url=${encodeURIComponent(url)}&username=${username}&text=${text}`)
		.then(buffer => {
			res.type("png").send(buffer);
		})
		.catch(e => {
...
router.get("/stickbug", apikeyAndLimit, async (req, res) => {
	...
	if (!regexUrl(url)) return res.json(loghandler.urlInvalid);
	try {
		let { data } = await axios.get(`https://nekobot.xyz/api/imagegen?url=${url}&type=stickbug`);
		if (isAxiosError()) return res.json(loghandler.error);
...
router.get("/tweet", apikeyAndLimit, async (req, res) => {
	...
	if (!text) return res.json(loghandler.nottext);
	try {
		buffer = await axios.get(`https://nekobot.xyz/api/imagegen?text=${text}&username=${query}&type=tweet`);
		if (isAxiosError()) return res.json(loghandler.error);
...
router.get("/animeface", apikeyAndLimit, async (req, res) => {
	...
	if (!regexUrl(url)) return res.json(loghandler.urlInvalid);
	try {
		let { data } = await axios.get(`https://nekobot.xyz/api/imagegen?image=${url}&type=animeface`);
		if (isAxiosError()) return res.json(loghandler.error);
...
router.get("/iphonex", apikeyAndLimit, async (req, res) => {
	...
	if (!regexUrl(url)) return res.json(loghandler.urlInvalid);
	try {
		let { data } = await axios.get(`https://nekobot.xyz/api/imagegen?type=iphonex&url=${url}`);
		if (isAxiosError()) return res.json(loghandler.error);
...
router.get("/kannagen", apikeyAndLimit, async (req, res) => {
	...
	try {
		let kannagen = await getBuffer(`https://nekobot.xyz/api/imagegen?raw=1&type=kannagen&text=${query}`);
		res.type("png");
		res.send(kannagen);
	} catch (er) {
		res.json(loghandler.error);
...
router.get("/captcha", apikeyAndLimit, async (req, res) => {
	...
	if (!regexUrl(url)) return res.json(loghandler.urlInvalid);
	try {
		let captcha = await getBuffer(`https://nekobot.xyz/api/imagegen?username=${query}&url=${url}&raw=1&type=captcha`);
		res.type("png");
		res.send(captcha);
	} catch (er) {
		res.json(loghandler.error);
...
router.get("/trumptweet", apikeyAndLimit, async (req, res) => {
	...
	if (!query) return res.json(loghandler.nottext)
	try {
		let trumptweet = await getBuffer(`https://canvacord-production.up.railway.app/api/trumptweet?text=${query}`);
		res.type("png");
		res.send(trumptweet);
	} catch (er) {
		res.json(loghandler.error);
...
router.get("/toonify", apikeyAndLimit, async (req, res) => {
	...
	if (!regexUrl(url)) return res.json(loghandler.urlInvalid);
	try {
		let toonify = await getBuffer(`https://canvacord-production.up.railway.app/api/toonify?url=${url}`);
		res.type("png");
		res.send(toonify);
	} catch (er) {
		res.json(loghandler.error);
...
router.get("/tahta", apikeyAndLimit, async (req, res) => {
	...
	if (!query) return res.json(loghandler.nottext)
	try {
		let Tahta = await getBuffer(`https://canvacord-production.up.railway.app/api/tahta?text=${query}`);
		res.type("png")
		res.send(Tahta);
	} catch (er) {
		res.json(loghandler.error);
...
router.get("/costumtahta", apikeyAndLimit, async (req, res) => {
	...
	try {
		let Tahta = await getBuffer(`https://canvacord-production.up.railway.app/api/costumtahta?text1=${query3}&text2=${query2}&text3=${query1}`);
		res.type("png");
		res.send(Tahta);
	} catch (er) {
		res.json(loghandler.error);
...
router.get("/karenhave", apikeyAndLimit, async (req, res) => {
	...
	if (!url) return res.json(loghandler.noturl)
	if (!regexUrl(url)) return res.json(loghandler.urlInvalid);
	ameApi.generate("lookwhatkarenhave", {
		url: url
	})
		.then(async image => {
			res.type("png")
...
router.get("/darkness", apikeyAndLimit, async (req, res) => {
	...
	if (!req.query.url) return res.json(loghandler.noturl)
	if (!regexUrl(req.query.url)) return res.json(loghandler.urlInvalid);
	await getBuffer("https://canvacord-production.up.railway.app/api/darkness?url=" + req.query.url + "&ammount=" + req.query.ammount)
		.then(buffer => {
			res.type("png")
			res.send(buffer)
		})
...
router.get("/fuse", apikeyAndLimit, async (req, res) => {
	...
	if (!regexUrl(url)) return res.json(loghandler.urlInvalid);
	if (!regexUrl(url2)) return res.json(loghandler.urlInvalid);
	await getBuffer("https://canvacord-production.up.railway.app/api/fuse?url=" + url + "&url2=" + url2)
		.then(buffer => {
			res.type("png").send(buffer)
		})
		.catch(e => {
...
router.get("/phub", apikeyAndLimit, async (req, res) => {
	...
	if (!text) return res.json(loghandler.nottext)
	if (!regexUrl(req.query.url)) return res.json(loghandler.urlInvalid);
	await getBuffer("https://canvacord-production.up.railway.app/api/phub?url=" + url + "&username=" + username + "&text=" + text)
		.then(async buffer => {
			res.type("png").send(buffer)
		})
		.catch(e => {
...
router.get("/ytcomment", apikeyAndLimit, async (req, res) => {
	...
	if (!text) return res.json(loghandler.nottext)
	if (!regexUrl(req.query.url)) return res.json(loghandler.urlInvalid);
	await getBuffer(`https://canvacord-production.up.railway.app/api/ytcomment?url=${url}&username=${username}&text=${text}`)
		.then(async buffer => {
			res.type("png").send(buffer)
		})
		.catch(e => {
...
router.get("/resize", apikeyAndLimit, async (req, res) => {
	...
	if (!url) return res.json(loghandler.noturl)
	if (!regexUrl(url)) return res.json(loghandler.urlInvalid);
	await getBuffer(`https://canvacord-production.up.railway.app/api/resize?url=${encodeURIComponent(req.query.url)}&height=${height}&width=${width}`).then(buffer => {
		res.type("png").send(buffer)
	}).catch(e => {
		console.error(e);
		res.json(loghandler.error);
...
router.get("/wanted", apikeyAndLimit, async (req, res) => {
	...
	if (!url) return res.json(loghandler.noturl)
	if (!regexUrl(url)) return res.json(loghandler.urlInvalid);
	await getBuffer("https://canvacord-production.up.railway.app/api/wanted?url=" + url)
		.then(buffer => {
			res.type("png")
			res.send(buffer)
		})
...
router.get("/wasted", apikeyAndLimit, async (req, res) => {
	...
	if (!url) return res.json(loghandler.noturl)
	if (!regexUrl(url)) return res.json(loghandler.urlInvalid);
	await getBuffer("https://canvacord-production.up.railway.app/api/wasted?url=" + url)
		.then(buffer => {
			res.type("png")
			res.send(buffer)
		})
...
router.get("/affect", apikeyAndLimit, async (req, res) => {
	...
	if (!url) return res.json(loghandler.noturl)
	if (!regexUrl(url)) return res.json(loghandler.urlInvalid);
	await getBuffer("https://canvacord-production.up.railway.app/api/affect?url=" + url)
		.then(buffer => {
			res.type("png")
			res.send(buffer)
		})
...
router.get("/gay", apikeyAndLimit, async (req, res) => {
	...
	if (!url) return res.json(loghandler.noturl)
	if (!regexUrl(url)) return res.json(loghandler.urlInvalid);
	await getBuffer("https://canvacord-production.up.railway.app/api/gay?url=" + url)
		.then(buffer => {
			res.type("png")
			res.send(buffer)
		})
...
router.get("/joke", apikeyAndLimit, async (req, res) => {
	...
	if (!url) return res.json(loghandler.noturl)
	if (!regexUrl(url)) return res.json(loghandler.urlInvalid);
	await getBuffer("https://canvacord-production.up.railway.app/api/joke?url=" + url)
		.then(buffer => {
			res.type("png")
			res.send(buffer)
		})
...
router.get("/opinion", apikeyAndLimit, async (req, res) => {
	...
	if (!url) return res.json(loghandler.noturl)
	if (!regexUrl(url)) return res.json(loghandler.urlInvalid);
	await getBuffer("https://canvacord-production.up.railway.app/api/opinion?url=" + url + "&text=" + text)
		.then(buffer => {
			res.type("png")
			res.send(buffer)
		})
...
router.get("/trash", apikeyAndLimit, async (req, res) => {
	...
	if (!url) return res.json(loghandler.noturl)
	if (!regexUrl(url)) return res.json(loghandler.urlInvalid);
	await getBuffer("https://canvacord-production.up.railway.app/api/trash?url=" + url)
		.then(buffer => {
			res.type("png")
			res.send(buffer)
		})
...
router.get("/facepalm", apikeyAndLimit, async (req, res) => {
	...
	if (!url) return res.json(loghandler.noturl)
	if (!regexUrl(url)) return res.json(loghandler.urlInvalid);
	await getBuffer("https://canvacord-production.up.railway.app/api/facepalm?url=" + url)
		.then(buffer => {
			res.type("png")
			res.send(buffer)
		})
...
router.get("/beautiful", apikeyAndLimit, async (req, res) => {
	...
	if (!url) return res.json(loghandler.noturl)
	if (!regexUrl(url)) return res.json(loghandler.urlInvalid);
	await getBuffer("https://canvacord-production.up.railway.app/api/beautiful?url=" + url).then(buffer => {
		res.type("png").send(buffer)
	}).catch(e => {
		console.error(e);
		res.json(loghandler.error);
...
router.get("/nulis2", apikeyAndLimit, async (req, res) => {
	...
	if (!Number(kelas)) return resError(res, "parameter kelas harus berupa angka");
	if (nama.length > 25) return resError(res, "text parameter nama terlalu panjang max 25 huruf");
	await getBuffer(`https://canvacord-production.up.railway.app/api/nulis?text=${text}&kelas=${kelas}&nama=${nama}`).then(buffer => {
		res.type("jpg")
		res.send(buffer)
	}).catch(e => {
		console.error(e);
...
router.get("/tts", apikeyAndLimit, async (req, res) => {
	...
	if (!text) return res.json(loghandler.nottext)
	tools.text2speach(text, language).then(async result => {
		await getBuffer(result).then(buffer => {
...
router.get("/ttp2", apikeyAndLimit, async (req, res) => {
	...
	await getBuffer(`https://canvacord-production.up.railway.app/api/ttp?text=${text}&color=${color}`)
		.then(async buffer => {
			res.type("png").send(buffer);
		})
		.catch(e => {
...
router.get("/welcome2", apikeyAndLimit, async (req, res) => {
	...
	await getBuffer(`https://canvacord-production.up.railway.app/api/welcome2?name=${name}&ucapan=${ucapan}&memcount=${memcount}&gcname=${gcname}&pp=${pp}&ppgc=${ppgc}`).then(data => {
		res.type("png")
		res.send(data)
	}).catch(e => {
		console.error(e);
...
router.get("/goodbye2", apikeyAndLimit, async (req, res) => {
	...
	if (!pp) return resError(res, "masukkan parameter pp");
	if (!ppgc) return resError(res, "masukkan parameter ppgc");
	await getBuffer(`https://canvacord-production.up.railway.app/api/goodbye2?name=${name}&ucapan=${ucapan}&memcount=${memcount}&gcname=${gcname}&pp=${pp}&ppgc=${ppgc}`).then(data => {
		res.type("png")
		res.send(data)
	}).catch(e => {
		console.error(e);
...
router.get("/welcome", apikeyAndLimit, async (req, res) => {
	...
	await getBuffer(`https://hadi-api.herokuapp.com/api/card/welcome?nama=${name}&descriminator=${ucapan}&memcount=${memcount}&gcname=${gcname}&pp=${pp}&bg=${bg}`).then(response => {
