...
router.get("/profile", isAuthenticated, async (req, res) => {
	...
	let identifikasiMember = 'Powered By Farhannnnn';
	res.render('profile', {
...
router.post("/register", async (req, res, next) => {
	try {
		...
		return validasi(req, res, "API XCODERS || VERIFICATION", next);
	} catch (err) {
		console.log(err);
...
router.post("/updates", async (req, res) => {
	...
	let bool = ["false", "true"];
	const requests = (status, result) => res.json({ status, creator: "Farhannnnn", result });
...
router.post("/settings", isAuthenticated, async (req, res) => {
	...
	if (username.length < 1 && apikey.length < 1 && profile == undefined) {
		...
		await User.findOneAndUpdate({ _id: users.id }, { username: username, apikey: apikey });
		...
		if (!/image|images/.test(profile.mimetype)) {
			...
			uploadByBuffer(profile.data, profile.mimetype)
				.then(async ({ link }) => {
					await User.findOneAndUpdate({ _id: users.id }, { username: username, url: link });
		...
		if (!/image|images/.test(profile.mimetype)) {
			...
			uploadByBuffer(profile.data, profile.mimetype)
				.then(async ({ link }) => {
					await User.findOneAndUpdate({ _id: users.id }, {
						username: username,
						apikey: apikey,
						url: link
					});
